...
function UserRegistration({
  ...
  const handleRegister = async (
    ...
    if (username.length < 4 || password.length < 4) {
      toast.error("Username and Password should be at least 4 character long");
      return;
    }
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        toast.success(
          response.data?.message || "Successfully registered employee"
        );
      ...
      <div className="flex flex-col w-2/5 2xl:w-1/3 bg-slate-50 rounded-2xl p-6">
        ...
        <div className="flex flex-col justify-start mt-5 space-y-2">
          ...
          <div className="space-y-1">
            ...
            <Input
              value={employee?.cost_center_code}
              disabled
              className="focus:border-none border-black"
            ></Input>
          </div>
