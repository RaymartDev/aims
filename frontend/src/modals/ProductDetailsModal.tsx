...
function ProductDetailsModal({ onClose, material ,onAdd }: ProductDetailsModalProps) {
    ...
    const handleAdd = () => {
        if (material) {
            onAdd(material);  // Pass the material back to the parent when the Add button is clicked
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-20 p-4">
            <div className="flex flex-col w-2/5 2xl:w-2/3 bg-slate-50 rounded-2xl p-6">
                <div className="flex items-center justify-between w-full border-b-2 border-black">
                    <h1 className="font-extrabold text-xl">Item Details</h1>
                    <Button
                ...
                <div className="mt-5 flex justify-end">
                    <Button className=" w-32 bg-hoverCream text-fontHeading font-semibold hover:text-white" onClick={handleAdd}>Add</Button>
                </div>
            </div>
        </div>
    );
