...
function ExportInventory({ onClose }: ExportInventoryProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${getVersion()}/inventory/export`, {
        params: {
          start: fromDate,
          end: toDate,
        },
      ...
      link.href = url;
      link.setAttribute("download", `Inventory_Report_${fromDate}_${toDate}.xlsx`);
    ...
    <div className={`fixed inset-0 bg-black bg-opacity-75 flex justify-center z-50 items-center`}>
      <div className="bg-slate-50 flex flex-col border border-black shadow-xl rounded-xl px-4 py-3">
        <div className="w-full flex justify-between px-2 py-2 border-b-2 border-gray-400">
          <h1 className="text-lg text-black font-bold">EXPORT INVENTORY</h1>
