...
function EditCategoryModal({ onClose, category, updateCategory }: EditCategoryModalProps) {
  const [description, setDescription] = useState(category?.description || '');

  const handleUpdate = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();
    try {
      const response = await axios.put(`${getVersion()}/material-category/update/${category?.id || 1}`, {
        description,
      })
